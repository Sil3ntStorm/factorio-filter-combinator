...
if not config['debug_mode'] then
    ...
    cc.flags = {'placeable-off-grid', 'not-repairable', 'not-on-map', 'not-deconstructable', 'not-blueprintable', 'hide-alt-info', 'not-flammable', 'no-copy-paste', 'not-selectable-in-game', 'not-upgradable', 'not-in-kill-statistics', 'not-in-made-in'}
    cc.draw_circuit_wires = false
    cc.sprites = util.empty_sprite(1)
    cc.selection_box = nil
end
cc.collision_box = nil
cc.collision_mask = {layers={}, not_colliding_with_itself = true}
cc.activity_led_light_offsets = { { 0, 0 }, { 0, 0 }, { 0, 0 }, { 0, 0 } }
cc.activity_led_sprites = { north = util.empty_sprite(1), south = util.empty_sprite(1), east = util.empty_sprite(1), west = util.empty_sprite(1)}
